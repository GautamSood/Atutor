...
const cloudinary = require("../utilities/CourseMaterial/Cloudinary")
...
exports.updateCourseMaterial = async(req,res) => {
    ...
    try {
        ...
        const newCourseMaterial = {
            ...
            courseId:courseId,
            courseMaterialLink:result.secure_url,
            courseMaterialTitle:req.file.originalname,
            createdAt:Date.now(),
        }
        const updatedCourseMaterial = await CourseMaterial.create(newCourseMaterial);
        const updatedCourse = await Course.findByIdAndUpdate(courseId,{$push:{courseMaterial:updatedCourseMaterial._id}},{new:true}).session(session);
        const updatedInstructor = await Instructor.findByIdAndUpdate(instructor._id,{$push:{uploadedCourseMaterial:updatedCourseMaterial._id}},{new:true}).populate("uploadedCourseMaterial").session(session);
        session.commitTransaction()
        return res.status(200).json({
            status:"success",
            result,
...
exports.deleteCourseMaterial = async(req,res,next) => {
    ...
    try {
        session.startTransaction();
        const {instructor} = req;
        const {courseMaterialId,courseId} = req.params;
        const courseMaterial = await CourseMaterial.findById(courseMaterialId);
        const updatedCourse = await Course.findByIdAndUpdate(courseId,{$pull:{courseMaterial:courseMaterialId}},{new:true}).session(session);
        const updatedInstructor = await Instructor.findByIdAndUpdate(instructor?._id,{$pull:{uploadedCourseMaterial:courseMaterialId}},{new:true}).session(session); 
        await cloudinary.uploader.destroy(courseMaterial?.cloudinaryId);
        await CourseMaterial.findByIdAndDelete(courseMaterialId).session(session);
        session.commitTransaction();
        return res.status(204).json({
            status:"success",
            data:null
